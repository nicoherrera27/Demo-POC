<script lang="ts">
 ...
 let poster = movie.poster_path ? `https://image.tmdb.org/t/p/w342$
{movie.poster_path}` : '';
 function toggleSeen() {
 const key = 'seen_' + movie.id;
 const current = localStorage.getItem(key) === '1';
 localStorage.setItem(key, current ? '0' : '1');
 window.dispatchEvent(new CustomEvent('movie:toggleSeen', { detail: { id:
movie.id } }));
 }
...
<div class="rounded-lg overflow-hidden shadow">
 ...
 <button on:click={toggleSeen} class="px-2 py-1 border rounded">Marcar
vista</button>
 <button on:click={addWatchlist} class="px-2 py-1 bg-amber-500 rounded

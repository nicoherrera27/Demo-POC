<script lang="ts">
  ...
  const updateFromStore = async (): Promise<void> => {
    try {
      ...
      alert('Svelte: Error updating from store:' + error);
    }
  };
  ...
  onMount(async () => {
    ...
    try {
      ...
      alert('Svelte: Error subscribing to store:' + error);
    }
...
<div class="bg-gradient-to-br from-green-900 via-teal-900 to-blue-900 p-6 rounded-lg shadow-lg">
  <div class="flex items-center justify-between mb-6">
    ...
    <div class="flex gap-2">
      <select bind:value={filter} class="bg-gray-700 text-white px-3 py-1 rounded text-sm border border-gray-600">
      ...
      <select bind:value={sortBy} class="bg-gray-700 text-white px-3 py-1 rounded text-sm border border-gray-600">
        ...
        <div class="bg-black bg-opacity-40 rounded-lg p-4 border-l-4 {item.isWatched ? 'border-green-500' : 'border-yellow-500'} transition-all hover:bg-opacity-60 border border-gray-700 hover:border-gray-500">
          <div class="flex gap-3">
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="flex gap-2">
                <button on:click={() => toggleWatched(item.id, item.type)} class="px-2 py-1 rounded text-xs transition-all {item.isWatched ? 'bg-green-600 hover:bg-green-700' : 'bg-yellow-600 hover:bg-yellow-700'} text-white hover:scale-105">
                  {item.isWatched ? '✓ Vista' : 'Marcar vista'}
                </button>
                
                <button on:click={() => removeFromWatchlist(item.id, item.type)} class="px-2 py-1 bg-red-600 hover:bg-red-700 text-white rounded text-xs transition-all hover:scale-105">
                  Eliminar
                </button>
              </div>
            </div>
